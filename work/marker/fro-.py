...
def color_track(im,h_min,h_max):
    ...
    cv2.namedWindow("1hsv", 0) 
    cv2.imshow("1hsv",im_h)
    mask = cv2.inRange(im_h,h_min,h_max,)       # マスク画像の生成
    cv2.namedWindow("2mask1", 0) 
    cv2.imshow("2mask1",mask)
    mask = cv2.medianBlur(mask,7)               # 平滑化
    cv2.namedWindow("3medianblur", 0) 
    cv2.imshow("3medianblur",mask)
    mask = cv2.dilate(mask,k,iterations=2)      # 膨張化
    cv2.namedWindow("4dilate", 0) 
    cv2.imshow("4dilate",mask)
...
def main():
    ...
    if n != -1:
        ...
        cv2.namedWindow("5drawContours", 0) 
        cv2.imshow("5drawContours",mask2)
        cv2.drawContours(im,[hull],0,(0,200,100),2)
        cv2.namedWindow("6drawContours", 0) 
        cv2.imshow("6drawContours",im)
        cv2.drawContours(mask2,[hull],0,(0,200,0),2)
        cv2.namedWindow("7drawContours", 0) 
        cv2.imshow("7drawContours",mask2)
