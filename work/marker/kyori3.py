...
def color_track(im,h_min,h_max):
    ...
    # 処理時間算出
    hsvtime = (end - start)/cv2.getTickFrequency()
    print("HSV色空間" + str(hsvtime) + "[sec]")

    start = cv2.getTickCount()
    mask = cv2.inRange(im_h,h_min,h_max,)       # マスク画像の生成
    end = cv2.getTickCount()
    # 処理時間算出
    msktime = (end - start)/cv2.getTickFrequency()
    print("マスク画像の生成" + str(msktime )+ "[sec]")

    start = cv2.getTickCount()
    mask = cv2.medianBlur(mask,7)               # 平滑化
    end = cv2.getTickCount()
    # 処理時間算出
    heikatutime = (end - start)/cv2.getTickFrequency()
    print("平滑化" + str(heikatutime) + "[sec]")

    start = cv2.getTickCount()
    mask = cv2.dilate(mask,k,iterations=2)      # 膨張化
    end = cv2.getTickCount()
    # 処理時間算出
    boutyoutime = (end - start)/cv2.getTickFrequency()
    print("膨張化" +str( boutyoutime) + "[sec]")

    start = cv2.getTickCount()
    im_c = cv2.bitwise_and(im,im,mask=mask)     # 色領域抽出
    end = cv2.getTickCount()
    # 処理時間算出
    irotime = (end - start)/cv2.getTickFrequency()
    print("色領域抽出" + str(irotime) + "[sec]")
...
def main():
    ...
    # 入力画像の取得
    start = cv2.getTickCount()
    im =   cv2.imread('20.jpg')      # カラーボールのトラッキング
    end = cv2.getTickCount()
    # 処理時間算出
    intime = (end - start)/cv2.getTickFrequency()
    print("入力" + str(intime) + "[sec]")
    ...
    # 処理時間算出
    rinkakutime = (end - start)/cv2.getTickFrequency()
    print("輪郭抽出" + str(rinkakutime) + "[sec]")
    ...
    if len(y)!= 0:
        ...
        # 処理時間算出
        keitime = (end - start)/cv2.getTickFrequency()
        print("奥行き計算" + str(keitime) + "[sec]")
        # 結果表示
        ...
        allend = cv2.getTickCount()
        # 処理時間算出
        alltime = (allend - allstart)/cv2.getTickFrequency()
