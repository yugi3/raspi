...
def    main():
    ...
    for i in natsorted(files):
        ...
        h1 = 104  # 測定場所での画面サイズ 220 [ピクセル]
        H =  50   # ボールの直径[mm]
        Z =  200  # 200mm先のz位置[mm
        # 入力画像の取得
        print(i)
        time_list.append(['画像取得', time.perf_counter()])
        im =   cv2.imread(i)      # カラーボールのトラッキング
        time_list.append(['画像取得', time.perf_counter()])
        ...
        if len(y)!= 0:
            ...
            mname = 'm' + i[9:]
            print(cname)
            cv2.imwrite(os.path.join(dirname[:8], cname),im)
            cv2.imwrite(os.path.join(dirname[:8], mname),mask2)
        
        
    #処理csvファイル作成
    syori = dirname[0]+ '処理時間.csv'
